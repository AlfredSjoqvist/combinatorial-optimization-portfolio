...
#-------------------------------------------------------------------------------
@constraint(SUDO, fix1[i in 1:9, j in 1:9; given[i,j] > 0],
    x[i, j, given[i, j]] == 1 )

@constraint(SUDO, fix2[i in 1:9, k in 1:9], sum(x[i, j,k] for j in 1:9) == 1)
@constraint(SUDO, fix3[j in 1:9, i in 1:9], sum(x[i,j,k] for k in 1:9) == 1)
@constraint(SUDO, fix4[j in 1:9, k in 1:9], sum(x[i,j,k] for i in 1:9) == 1)
@constraint(SUDO, fix5[k in 1:9, r in 0:2, c in 0:2], sum(x[i,j,k] for i in 1+r*3:3+r*3 for j in 1+c*3:3+c*3) == 1)
